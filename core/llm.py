...
def get_model(high_context=False, the_model=None):
    ...
    the_nexusai_api_key=load_nexusai_api_key()
    the_nexusai_url=load_nexusai_url()
    ...
    for model_name, model_args in llm_settings.items():
        ...
        elif model_args["provider"] == "NexusAI":
            the_tuple = (ChatOpenAI, {
                "model": the_model,
                "api_key": the_nexusai_api_key,
                "openai_api_base":the_nexusai_url.strip('/')+"/api"
            })
        if the_tuple:
