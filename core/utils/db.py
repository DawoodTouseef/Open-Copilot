...
def load_anthropic_api_key():
    if kot_db_.get("anthropic_api_key"):
        ...
        if env_variable:
            ...
            return env_variable
        return "CHANGE_ME"
def save_nexusai_api_key(api_key):
    kot_db_.set("nexusai_api_key", api_key)
def load_nexusai_api_key():
    if kot_db_.get("nexusai_api_key"):
        return kot_db_.get("nexusai_api_key")
    else:
        env_variable = os.getenv("NEXUSAI_API_KEY")
        if env_variable:
            save_api_key(env_variable)
            return env_variable
        return "CHANGE_ME"
...
def save_nexusai_url(url):
    kot_db_.set("nexusai_url", url)

def load_nexusai_url():
    if kot_db_.get("nexusai_url"):
        return kot_db_.get("nexusai_url")
    else:
...
def load_system_prompt():
    if kot_db_.get("system_prompt"):
        ...
        return "Hi, you are an JARVIS(Just A Rather Very Intelligent System). You need to understand the user aspect and then trying to do these things and give valuation."
