try:
    ...
    from .tts_providers.system_voice import tts_pyttsx3
except ImportError:
    from core.llm import *
    from core.utils.db import *
    from core.audio.tts_providers.openai import tts_openai
    from core.audio.tts_providers.microsoft_local import tts_microsoft_local
    from core.audio.tts_providers.system_voice import tts_pyttsx3
...
def generate_speech_chunk(text_chunk, index, voice, results):
    ...
    if os.path.exists(location):
        ...
        if tts_setting == 'NexusAI':
            from core.audio.tts_providers.nexusai import tts_nexusai
            tts_nexusai(text_chunk, location)
        if tts_setting == 'System':
            tts_pyttsx3(text_chunk,location)
        results[index] = location
